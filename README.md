...
**Estado:** publicada la sección 01 (ventas por canal). Las secciones 02
(devoluciones que llegan tarde) y 03 (margen de contribución) están en curso.

## Por dónde empezar

1. **El report**, que es el entregable: [`report/index.html`](report/index.html).
   Es un fichero HTML autocontenido —lleva los gráficos dentro y funciona sin
   conexión—, así que se abre haciendo doble clic. Para leerlo en GitHub sin
   descargar nada, el mismo texto está en
   [`seccion_01_canales_report.md`](seccion_01_canales_report.md).
...
| Carpeta | Qué hay |
|---|---|
| `report/` | `build_report.py` monta el HTML a partir de los `.md` y de los marts. El texto no calcula nada: cada cifra sale de una tabla de dbt. |
...
[`seccion_01_canales.md`](seccion_01_canales.md), que es el plan de la sección 01
escrito **antes** de la primera consulta para que el análisis no derivara hacia lo
que resultara fácil de sacar.
...
make report   # regenera report/index.html
```
